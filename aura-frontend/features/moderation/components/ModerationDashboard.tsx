...
import { AlertTriangle, CheckCircle2, Clock, RefreshCcw, ShieldAlert, UserCog, User } from "lucide-react";
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-6 py-10">
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* Role toggle — replace with JWT claim in Phase 6 */}
          <button
            type="button"
            onClick={() => setRole(role === "admin" ? "user" : "admin")}
            title="Toggle role (dev only — no auth yet)"
            className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300 transition hover:bg-white/10"
          >
            {isAdmin
              ? <><UserCog className="h-3.5 w-3.5 text-indigo-400" aria-hidden /> Admin</>
              : <><User className="h-3.5 w-3.5 text-slate-400" aria-hidden /> User</>
            }
          </button>
          ...
          {error ? (
            <Badge variant="danger" className="max-w-xs truncate">{error}</Badge>
          ) : null}
                ...
                {!loading && images.length === 0 ? (
                  <tr>
                    <td className="px-2 py-10 text-center text-sm text-slate-400" colSpan={6}>
                      No images yet. Upload one above to start scanning.
                    </td>
                  </tr>
                ) : null}
              </tbody>
      ...
      {/* User view — waiting state after upload */}
      {!isAdmin && (
        <Card>
          <CardContent className="py-8 text-center text-sm text-slate-400">
            Upload an image above. Results are processed automatically.
            <br />
            <span className="mt-1 block text-xs text-slate-600">
              Switch to Admin view to see the moderation log.
            </span>
          </CardContent>
        </Card>
      )}
